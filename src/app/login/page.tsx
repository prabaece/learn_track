...
//     setEmail({
//       value: v,
//       error: email.touched ? validators.email(v) : "",
...
//     setEmail((p) => ({
//       ...p,
//       error: validators.email(p.value),
//       touched: true,
//     }));
...
//       setConfirm((p) => ({
//         ...p,
//         error: validators.confirmPassword(p.value, v),
//       }));
//     setPassword({
//       value: v,
//       error: password.touched ? validators.password(v) : "",
...
//     setPassword((p) => ({
//       ...p,
//       error: validators.password(p.value),
//       touched: true,
//     }));
...
//     setConfirm({
//       value: v,
//       error: confirm.touched
//         ? validators.confirmPassword(v, password.value)
//         : "",
...
//     setConfirm((p) => ({
//       ...p,
//       error: validators.confirmPassword(p.value, password.value),
//       touched: true,
//     }));
...
//     const emailErr = validators.email(email.value);
//     const passwordErr = validators.password(password.value);
//     const confirmErr = isSignup
//       ? validators.confirmPassword(confirm.value, password.value)
//       : "";
...
//             >
//               <button
//                 className={`tab-btn ${!isSignup ? "tab-active" : "tab-inactive"}`}
    ...
    <>
      <style>{`
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* ── FIX: Kill browser autofill white background ── */
        input::placeholder { color: rgba(255,255,255,0.25); }
        input:-webkit-autofill,
        ...
        .inp {
          ...
          font-family: 'DM Sans','Segoe UI',sans-serif;
          transition: border-color 0.2s;
          /* Prevent OS from painting its own background */
          color-scheme: dark;
        }
        .inp:focus { border-color: rgba(124,58,237,0.6); }
        .inp-err { border-color: rgba(239,68,68,0.5) !important; }
        ...
        <div className="login-card">
          ...
          >
            ...
            <div
              style={{
                display: "flex", background: "rgba(0,0,0,0.25)",
                borderRadius: 12, padding: 4, marginBottom: "1.25rem", gap: 4,
              }}
            >
              <button
                className={`tab-btn ${!isSignup ? "tab-active" : "tab-inactive"}`}
                    ...
                    {confirm.value && !confirm.error && (
                      <span
                        style={{
                          ...
                          transform: "translateY(-50%)", color: "#22c55e", fontSize: 14,
                        }}
                      >
                        ✓
                      </span>
                ...
                <div style={{ textAlign: "right", marginBottom: "0.75rem" }}>
                  <button
                    ...
                    onClick={async () => {
                      if (!email.value) {
                        setEmail((p) => ({
                          ...p, error: "Enter your email first", touched: true,
                        }));
                        return;
                      }
                      const { error } = await supabase.auth.resetPasswordForEmail(
                        email.value,
                        {
                          // ── FIX: redirect to the reset-password page, not root ──
                          redirectTo: `${window.location.origin}/reset-password`,
                        }
                      );
                      if (!error)
                        setServerSuccess("Password reset email sent! Check your inbox.");
                      else setServerError(error.message);
                    }}
                  >
                    Forgot password?
